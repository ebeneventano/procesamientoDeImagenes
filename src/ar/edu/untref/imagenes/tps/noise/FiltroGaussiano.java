...
public class FiltroGaussiano {
	...
	public static BufferedImage aplicarFiltroLoG(BufferedImage imagenOriginal, int sigma) {

		float[][] mascara = generarMascaraLoG(sigma);
	...
	private static float[][] generarMascaraLoG(int sigma) {
		...
		for (int j = 0; j < dimension; ++j) {
			for (int i = 0; i < dimension; ++i) {
				mascara[i][j] = calcularValorLoG(sigma, i - (dimension/2), j - (dimension/2));
			}
		}
	...
	private static float calcularValorLoG(int sigma, int x, int y) {
		float valor = (float) ((1 / (Math.sqrt(2) * Math.PI * sigma * sigma * sigma)) * (2 - ((x * x) + (y * y))/(sigma * sigma))
				* 
